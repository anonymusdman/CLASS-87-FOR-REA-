var canvas = new fabric.Canvas("not_backgorund");
var steve_x = 10;
var steve_y = 10;
...
function keydown_event(e) {
    ...
    function steve_up(){
        if(steve_y >= 0){
            steve_y = steve_y - block_building_height;
            canvas.remove(steve_object);
            console.log("steveupisworking")
            steveUpdate();
        }
    }
    function steve_left(){
        if(steve_x > 0){
            steve_x = steve_x - block_building_height;
            canvas.remove(steve_object);
            console.log("steveleftisworking");
            steveUpdate();
        }
    }
    function steve_right(){
        if(steve_x <= 1000){
            steve_x = steve_x + block_building_height;
            canvas.remove(steve_object);
            console.log("steverightisworking");
            steveUpdate();
        }
    }
    function steve_down(){
        if(steve_y <= 600){
            steve_y = steve_y + block_building_height;
            canvas.remove(steve_object);
            console.log("stevedownisworking");
            steveUpdate();
        }
    }
}